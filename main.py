...
# 仅抑制裸 IP 检测产生的证书告警，避免刷屏；这不代表关闭 TLS 校验。
# 除 check_http_server 中按裸 IP 直连的请求外，所有请求都必须校验证书：
# Cloudflare API / WxPusher / 数据源 / 校准 API 均携带密钥，绝不能接受中间人。
...
def send_wxpusher_notification(content, summary):
    ...
    try:
        ...
        # WxPusher 在 appToken 无效时同样返回 HTTP 200，必须校验响应体中的 code
        try:
            resp_code = resp.json().get("code")
        except Exception:
            resp_code = None
        if resp.status_code == 200 and resp_code == 1000:
            print("微信通知已发送")
        else:
            print(f"微信通知发送失败: HTTP {resp.status_code}, code={resp_code}")
...
def measure_bandwidth_curl(node_str):
    ...
    curl_cmd = [
        ...
        # 不使用 --http2：Windows 自带的 Schannel 版 curl 无 HTTP/2 支持，
        # 带上该参数会让 curl 直接以 exit=2 退出，导致所有节点带宽记为 0。
        "--noproxy", "*",
