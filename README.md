...
| `git_sync.ps1` | Windows 推送脚本（只把 `ip.txt` 推送到 GitHub，不拉取代码） |
| `git_sync.sh` | Linux 推送脚本（只把 `ip.txt` 推送到 GitHub，不拉取代码） |
...
3. 赋予推送脚本执行权限（如果需要）：
   ...
   chmod +x git_sync.sh
   ```
4. （可选）添加 cron 任务（以当前普通用户身份运行，不要用 root）：
   ```bash
   (crontab -l 2>/dev/null; echo "*/5 * * * * cd $(pwd) && /usr/bin/python3 $(pwd)/main.py >> $(pwd)/cron.log 2>&1") | crontab -
   ```
5. 验证：`crontab -l`
