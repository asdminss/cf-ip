...
# 注意：ip.txt 是优选结果，需要提交并推送到仓库，故不在此忽略
EOF
...
# ---------- 确定定时任务归属用户（避免以 root 运行 main.py） ----------
# 使用 sudo 运行时，任务写入原始用户而非 root；否则写入当前用户。
# 这样 main.py / git_sync.sh 一旦被篡改，也无法获得 root 权限。
if [[ $EUID -eq 0 && -n "${SUDO_USER:-}" && "$SUDO_USER" != "root" ]]; then
    CRON_TARGET_USER="$SUDO_USER"
    CRON_USER_ARGS=(-u "$SUDO_USER")
else
    CRON_TARGET_USER="$(id -un)"
    CRON_USER_ARGS=()
fi
echo -e "   定时任务归属用户: ${CYAN}$CRON_TARGET_USER${NC}（非 root，降低提权风险）"
...
if crontab "${CRON_USER_ARGS[@]}" -l 2>/dev/null | grep -F "$SCRIPT_DIR/$PYTHON_SCRIPT" > /dev/null; then
    echo -e "${YELLOW}⚠️ 定时任务已存在，跳过添加。${NC}"
else
    (crontab "${CRON_USER_ARGS[@]}" -l 2>/dev/null || true; echo "$CRON_COMMENT"; echo "$CRON_CMD") | crontab "${CRON_USER_ARGS[@]}" -
    echo -e "${GREEN}✅ 定时任务已添加（每${TASK_INTERVAL_MINUTES}分钟，从下一个整5分钟开始）${NC}"
fi

echo -e "   执行命令: $PYTHON_PATH $SCRIPT_DIR/$PYTHON_SCRIPT（用户 $CRON_TARGET_USER）"
...
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    echo -e "${CYAN}正在运行 main.py ...（以 $CRON_TARGET_USER 身份）${NC}"
    # 以目标用户运行，避免生成 root 属主的 ip.txt / .run.lock 等文件，
    # 否则后续非 root 的定时任务将无权读写。
    if [[ $EUID -eq 0 && "$CRON_TARGET_USER" != "root" ]]; then
        su "$CRON_TARGET_USER" -c "cd \"$SCRIPT_DIR\" && \"$PYTHON_PATH\" \"$PYTHON_SCRIPT\""
    else
