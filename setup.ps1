...
try {
    ...
    # 以当前用户身份运行（S4U：无人登录时也能执行），不使用 SYSTEM 高权限。
    # 否则该目录一旦被非特权用户篡改，main.py / git_sync.ps1 就会以 SYSTEM 执行任意代码。
    ...
    # 单次运行约 15~20 分钟（TCP/可用性/HTTP/带宽多阶段），触发间隔仅 5 分钟。
    # 若用 StopExisting(3)，正在跑的实例会被下一个触发直接杀掉，永远跑不到写 DNS 那一步；
    # 必须用 IgnoreNew(2)：已在运行则跳过本次触发，等下一轮再试。
    # 同时把运行时限从 72 小时收紧到 1 小时，防止某次卡死导致后续触发被无限期跳过。
    ...
    Write-Host "   运行账户: $env:USERDOMAIN\$env:USERNAME (S4U，无需密码/无人登录时也运行，非 SYSTEM)" -ForegroundColor Gray
    ...
    # /IT：以当前用户的交互式令牌运行，无需在命令行提供密码，也不使用 SYSTEM / 最高权限。
    $schtasksArgs = @(
        "/Create",
        "/TN", $TaskName,
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ 计划任务 '$TaskName' 创建成功！" -ForegroundColor Green
        Write-Host "   创建方式: schtasks（以当前用户运行，仅在登录状态下执行）" -ForegroundColor Gray
    } else {
