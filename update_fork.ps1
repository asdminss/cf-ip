...
Write-Host "`n[3/6] 拉取远程并快进对齐" -ForegroundColor Yellow
git fetch origin $Branch
# 不再使用 git reset --hard：它会无声丢弃本地未提交的修改（包括安全修复），且无法恢复。
if (git status --porcelain) {
    Write-Host "错误：存在未提交的本地修改，为避免被覆盖已中止。请先 git commit 或 git stash。" -ForegroundColor Red
    Read-Host "按 Enter 键退出"
    exit 1
}
git merge --ff-only "origin/$Branch"
if ($LASTEXITCODE -ne 0) {
    Write-Host "错误：无法快进合并（本地与远端已分叉）。脚本不会强制覆盖，请人工处理。" -ForegroundColor Red
    Read-Host "按 Enter 键退出"
    exit 1
}
...
Write-Host "`n[5/6] 更新 git_sync.ps1 和 git_sync.sh（仅替换令牌，不注入 git pull）" -ForegroundColor Yellow
# 以下 here-string 必须用单引号 @'...'@：双引号会让 PowerShell 把 Python 代码里的
# $github_token / $branch 展开成空，替换后会把 git_sync.ps1 的凭据行写坏。
...
def update_file(filename, pattern_map):
    try:
    ...
    # 不再向 git_sync 注入 git pull：该目录会被定时任务执行，
    # 拉取远端代码等于把"仓库写入权限"变成"本目录任意代码执行"。
...
update_file('git_sync.ps1', ps_patterns)
...
update_file('git_sync.sh', sh_patterns)
